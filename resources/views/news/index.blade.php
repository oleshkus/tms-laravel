<x-layout>
    <x-header>
        <div class="flex items-center text-center">
            <a href="{{route('news.create')}}" class="text-white hover:text-opacity-90 text-opacity-50 bg-white p-2 px-3 rounded-md
    ...
    <div class="container flex flex-row flex-wrap justify-between items-start">
        ...
        @foreach($news as $new)
            <x-news_card :title="$new->title" :created="$new->created_at" :content="$new->content"
                         :author="$new->author" :category="$new->category" :news="$new">
            </x-news_card>
        @endforeach
    </div>
